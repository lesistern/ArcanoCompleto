...
import { PostEditor } from '@/components/tiptap/PostEditor';
  ...
  useEffect(() => {
    const loadProfile = async () => {
      ...
      const { data: profile } = await supabase
        .from('profiles')
        .select('tags, patreon_tier, is_patreon, tier_code, roles')
        .eq('id', user.id)
        .single();
      const tagsArr = Array.isArray(profile?.tags) ? profile?.tags : [];
      const hasCollaborator = tagsArr.some((t: string) => t?.toLowerCase() === 'colaborador');
      const isPatreon = Boolean(profile?.is_patreon || profile?.patreon_tier || profile?.tier_code === 'patreon');
      const roles = Array.isArray(profile?.roles) ? profile.roles : [];
      const hasRoleCollaborator = roles.some((r: string) => r?.toLowerCase() === 'colaborador');
      setCanUploadImages(isPatreon || hasCollaborator || hasRoleCollaborator);
    };
    loadProfile();
  }, [supabase]);
    ...
    <form onSubmit={handleSubmit} className="p-6 lg:p-8 space-y-8">
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <label className="text-sm font-bold text-dungeon-200 uppercase tracking-wider">Categoría</label>
      ...
      <div className="space-y-2">
        <label className="text-sm font-bold text-dungeon-200 uppercase tracking-wider">Título</label>
        <div className="relative group">
          <input
            id="title"
            type="text"
            className="w-full rounded-xl border border-dungeon-700 bg-dungeon-900/50 px-5 py-4 text-lg font-medium text-dungeon-100 placeholder:text-dungeon-600 focus:outline-none focus:ring-2 focus:ring-gold-500 focus:border-transparent transition-all shadow-inner"
            value={title}
            ...
            required
          />
          <div className="absolute right-3 bottom-3 text-xs text-dungeon-600 font-mono bg-dungeon-900/80 px-2 py-1 rounded">
        ...
        <div className="flex items-center justify-between border-b border-dungeon-800 pb-1">
          <div className="flex gap-4">
            <button
              ...
              className={`flex items-center gap-2 pb-2 text-sm font-semibold transition-colors ${activeTab === 'editor' ? 'text-gold-400 border-b-2 border-gold-400' : 'text-dungeon-500 hover:text-dungeon-300'
                }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-2 pb-2 text-sm font-semibold transition-colors ${activeTab === 'preview' ? 'text-gold-400 border-b-2 border-gold-400' : 'text-dungeon-500 hover:text-dungeon-300'
                }`}
            >
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className={`text-xs flex items-center gap-1 px-2 py-1 rounded transition-colors ${showPoll ? 'text-gold-400 bg-gold-400/10' : 'text-dungeon-500 hover:text-dungeon-300'}`}
            >
            ...
            <button
              type="button"
              onClick={clearDraft}
              className="text-xs flex items-center gap-1 text-dungeon-600 hover:text-red-400 transition-colors px-2 py-1"
      ...
      {/* Tags Input */}
      <div className="space-y-2">
        <label htmlFor="tags" className="text-sm font-bold text-dungeon-200 uppercase tracking-wider">
          Etiquetas <span className="text-dungeon-500 font-normal normal-case">(Opcional, máx 5)</span>
        </label>
        <div className="flex flex-wrap gap-2 p-2 rounded-xl bg-dungeon-900/50 border border-dungeon-800 focus-within:border-gold-500/50 transition-colors">
          {tags.map((tag) => (
            <span
              key={tag}
              className="inline-flex items-center gap-1 pl-3 pr-2 py-1 rounded-full bg-gold-500/10 text-gold-300 border border-gold-500/20 text-sm animate-in fade-in zoom-in duration-200"
            >
              #{tag}
              <button
                type="button"
                onClick={() => removeTag(tag)}
                className="p-0.5 hover:bg-gold-500/20 rounded-full transition-colors"
      ...
      <div className="flex items-center justify-end gap-4 pt-6 border-t border-dungeon-800">
        <button
          ...
          className="
            ...
            text-dungeon-950 font-bold text-lg shadow-lg shadow-gold-900/20
            hover:from-gold-500 hover:to-gold-400 hover:shadow-gold-500/20 hover:-translate-y-0.5
