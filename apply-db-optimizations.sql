...
-- ============================================================================
-- Propósito: Ejecutar todas las mejoras propuestas en SCHEMA_IMPROVEMENTS.md
-- Versión: 1.0
-- Fecha: 2025-01-14
-- ============================================================================
...
CREATE TABLE IF NOT EXISTS books (
  ...
  category TEXT NOT NULL, -- 'core', 'complete', 'monster_manual', 'supplement', 'setting', 'magazine'
  subcategory TEXT, -- 'eberron', 'forgotten_realms', 'dragonlance', 'dragon', 'dungeon'
  edition TEXT DEFAULT '3.5', -- '3.5' o '3.0-updated'
  priority TEXT DEFAULT 'medium', -- 'critical', 'high', 'medium', 'low', 'optional'
  description TEXT,
...
CREATE INDEX idx_books_priority ON books(priority);
...
COMMENT ON TABLE books IS 'Catálogo de libros de D&D 3.5 disponibles';
COMMENT ON COLUMN books.edition IS '3.5 para libros nativos, 3.0-updated para libros 3.0 actualizados a 3.5';
COMMENT ON COLUMN books.priority IS 'Prioridad de implementación: critical, high, medium, low, optional';

-- ============================================================================
-- PARTE 2: MEJORAS A LA TABLA WEAPONS
-- ============================================================================
...
UPDATE weapons SET
  range_feet = CAST(REGEXP_REPLACE(range_increment, '[^0-9]', '', 'g') AS INTEGER)
WHERE range_feet IS NULL AND range_increment IS NOT NULL;

UPDATE weapons SET
  damage_medium = damage
WHERE damage_medium IS NULL;

-- Extraer proficiency del weapon_type
UPDATE weapons SET
  proficiency = CASE
    WHEN weapon_type ILIKE '%simple%' THEN 'simple'
    WHEN weapon_type ILIKE '%marcial%' THEN 'marcial'
    WHEN weapon_type ILIKE '%exótica%' OR weapon_type ILIKE '%exotic%' THEN 'exótica'
    ELSE 'simple'
  END
WHERE proficiency IS NULL;

-- Extraer combat_category del weapon_type
UPDATE weapons SET
  combat_category = CASE
    WHEN weapon_type ILIKE '%distancia%' OR weapon_type ILIKE '%ranged%' THEN 'ranged'
    WHEN weapon_type ILIKE '%cuerpo a cuerpo%' OR weapon_type ILIKE '%melee%' THEN 'melee'
    ELSE category
  END
WHERE combat_category IS NULL;
...
ALTER TABLE weapons DROP CONSTRAINT IF EXISTS check_proficiency;
ALTER TABLE weapons ADD CONSTRAINT check_proficiency
  CHECK (proficiency IN ('simple', 'marcial', 'exótica') OR proficiency IS NULL);

ALTER TABLE weapons DROP CONSTRAINT IF EXISTS check_combat_category;
...
ALTER TABLE weapons DROP CONSTRAINT IF EXISTS check_hands;
...
COMMENT ON COLUMN weapons.proficiency IS 'Competencia requerida: simple, marcial, exótica';
COMMENT ON COLUMN weapons.combat_category IS 'Categoría de combate: melee, ranged, melee-ranged';
COMMENT ON COLUMN weapons.hands IS 'Manos requeridas: unarmed, light, one-handed, two-handed';

-- ============================================================================
-- PARTE 3: MEJORAS A LA TABLA SKILLS
-- ============================================================================

-- 1. Verificar y corregir datos de key_ability antes de aplicar constraint
-- Ver qué valores únicos hay actualmente
DO $$
BEGIN
  -- Mostrar valores actuales para debugging
  RAISE NOTICE 'Valores actuales de key_ability en la tabla skills:';
END $$;

-- Normalizar valores existentes (si hay alguna variación)
...
-- Ahora aplicar el constraint (solo si no existe)
ALTER TABLE skills DROP CONSTRAINT IF EXISTS check_key_ability;

-- Solo añadir constraint si todos los valores son válidos
DO $$
DECLARE
...
BEGIN
  ...
  IF invalid_count = 0 THEN
    ...
    RAISE NOTICE 'Constraint check_key_ability aplicado exitosamente';
  ELSE
    RAISE WARNING 'No se pudo aplicar constraint check_key_ability: % filas con valores inválidos', invalid_count;
  END IF;
END $$;
...
COMMENT ON COLUMN skills.class_skills IS 'Clases que tienen esta habilidad como class skill';
COMMENT ON COLUMN skills.example_dcs IS 'Ejemplos de DCs típicos en formato JSON: {"10": "Fácil", "15": "Normal"}';

-- ============================================================================
-- PARTE 4: MEJORAS A LA TABLA FEATS
-- ============================================================================

-- 1. Verificar y normalizar categorías antes de aplicar constraint
...
-- Aplicar constraint solo si los datos son válidos
...
BEGIN
  ...
  IF invalid_count = 0 THEN
    ...
    RAISE NOTICE 'Constraint check_feat_category aplicado exitosamente';
  ELSE
    RAISE WARNING 'No se pudo aplicar constraint check_feat_category: % filas con valores inválidos', invalid_count;
  END IF;
END $$;
...
COMMENT ON COLUMN feats.prerequisite_feats IS 'Array de dotes requeridas como prerequisito';
COMMENT ON COLUMN feats.prerequisite_bab IS 'BAB mínimo requerido';
COMMENT ON COLUMN feats.prerequisite_abilities IS 'Habilidades mínimas requeridas en formato JSON';
COMMENT ON COLUMN feats.prerequisite_skills IS 'Rangos de habilidades requeridos en formato JSON';

-- ============================================================================
-- PARTE 5: MEJORAS A LA TABLA CLASSES
-- ============================================================================

-- 1. Añadir tipo de clase primero (antes de constraints)
ALTER TABLE classes ADD COLUMN IF NOT EXISTS class_type TEXT DEFAULT 'base'; -- 'base', 'prestige', 'npc'

-- 2. Aplicar constraints solo si la tabla tiene datos válidos o está vacía
ALTER TABLE classes DROP CONSTRAINT IF EXISTS check_hit_die;
ALTER TABLE classes DROP CONSTRAINT IF EXISTS check_bab_progression;
ALTER TABLE classes DROP CONSTRAINT IF EXISTS check_save_progression;
ALTER TABLE classes DROP CONSTRAINT IF EXISTS check_class_type;

-- Solo aplicar si no hay clases aún (tabla vacía) o si los datos son válidos
DO $$
DECLARE
  class_count INTEGER;
BEGIN
  ...
  IF class_count = 0 THEN
    -- Tabla vacía, aplicar todos los constraints
    ...
    RAISE NOTICE 'Constraints de classes aplicados exitosamente (tabla vacía)';
  ELSE
    RAISE NOTICE 'Tabla classes tiene datos. Constraints no aplicados automáticamente para evitar errores';
  END IF;
END $$;

COMMENT ON COLUMN classes.class_type IS 'Tipo de clase: base (PH), prestige (clases de prestigio), npc (clases NPC)';

-- ============================================================================
-- PARTE 6: MEJORAS A LA TABLA RACES
-- ============================================================================

-- 1. Aplicar constraint de size solo si la tabla está vacía
ALTER TABLE races DROP CONSTRAINT IF EXISTS check_race_size;
...
BEGIN
  ...
  IF race_count = 0 THEN
    ...
    RAISE NOTICE 'Constraint check_race_size aplicado exitosamente';
  ELSE
    RAISE NOTICE 'Tabla races tiene datos. Constraint check_race_size no aplicado automáticamente';
  END IF;
END $$;
...
COMMENT ON COLUMN races.creature_type IS 'Tipo de criatura: Humanoide, Monstruoso Humanoide, etc.';
COMMENT ON COLUMN races.subtypes IS 'Subtipos raciales como Elfo, Enano, Goblinoide';
COMMENT ON COLUMN races.darkvision IS 'Rango de visión en la oscuridad en pies, NULL si no tiene';

-- ============================================================================
-- PARTE 7: MEJORAS A LA TABLA SPELLS
-- ============================================================================

-- 1. Aplicar constraint de school solo si la tabla está vacía
ALTER TABLE spells DROP CONSTRAINT IF EXISTS check_spell_school;
...
BEGIN
  ...
  IF spell_count = 0 THEN
    ...
    RAISE NOTICE 'Constraint check_spell_school aplicado exitosamente';
  ELSE
    RAISE NOTICE 'Tabla spells tiene datos. Constraint check_spell_school no aplicado automáticamente';
  END IF;
END $$;
...
COMMENT ON COLUMN spells.component_verbal IS 'Requiere componente verbal (V)';
COMMENT ON COLUMN spells.component_somatic IS 'Requiere componente somático (S)';
COMMENT ON COLUMN spells.component_material IS 'Requiere componente material (M)';
COMMENT ON COLUMN spells.spell_type IS 'Tipo de hechizo: arcane, divine, both';

-- ============================================================================
-- PARTE 8: NUEVA TABLA ARMOR (Armaduras)
-- ============================================================================
...
ALTER TABLE armor DROP CONSTRAINT IF EXISTS check_armor_type;
...
-- ============================================================================
-- PARTE 9: NUEVA TABLA MAGIC_ITEMS (Objetos Mágicos)
-- ============================================================================
...
-- ============================================================================
-- PARTE 10: NUEVA TABLA MONSTERS (Monstruos)
-- ============================================================================
...
-- Vista para libros por prioridad
CREATE OR REPLACE VIEW v_books_by_priority AS
SELECT
  priority,
  COUNT(*) as total_books,
  array_agg(name ORDER BY name) as book_names
FROM books
...
-- ============================================================================
-- PARTE 12: FUNCIONES DE UTILIDAD
-- ============================================================================
...
COMMENT ON FUNCTION calculate_bab IS 'Calcula el BAB para un nivel y progresión dados';
COMMENT ON FUNCTION calculate_save IS 'Calcula el bonus de salvación para un nivel y progresión dados';

-- ============================================================================
-- COMMIT DE TODAS LAS MEJORAS
-- ============================================================================
