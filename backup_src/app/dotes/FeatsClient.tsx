...
export default function FeatsClient({ feats }: FeatsClientProps) {
    ...
    const featsByType: Record<FeatType, DnDFeat[]> = {
        ...
        'Especial': filteredFeats.filter(f => f.type === 'Especial'),
    };

    // Ordenar tipos
    const typeOrder: FeatType[] = ['General', 'Combate', 'Metamagia', 'Creación de Objetos', 'Especial'];
            ...
            <ScrollReveal delay={100}>
                <div className="mb-6">
                    ...
                    >
                        ...
                        <ChevronDown
                            className={`h-5 w-5 text-gold-500 transition-transform ${filtersOpen ? 'rotate-180' : ''
                                }`}
                        />
                    </button>
