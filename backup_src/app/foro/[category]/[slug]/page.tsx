...
async function loadThreadData(categorySlug: string, threadSlug: string, page: number, pageSize: number) {
  ...
  if (!threadRow) {
    return { thread: null, rootPost: null, replies: [], currentUserId };
  }

  // Registrar la vista mediante trigger de conteo
  await supabase
  ...
  const { data: repliesData, count: repliesCount } = await repliesQuery.range(offset, offset + pageSize - 1);
