...
export async function generateStaticParams() {
    ...
    const [
        ...
        supabase.from('alignments').select('slug'),
    ...
    if (alignments) {
        params.push(...alignments.map(i => ({ category: 'alineamientos', slug: i.slug })));
    }
    if (dice) {
...
export default async function RuleDetailPage({ params }: PageProps) {
    ...
    return (
        <div className="container mx-auto px-4 py-12 max-w-4xl">
            ...
            <Card className="bg-dungeon-800 border-dungeon-700 overflow-hidden">
                ...
                <CardContent className="space-y-6">
                    ...
                    {category === 'alineamientos' && (
                        <div className="flex items-center gap-4 p-4 bg-dungeon-900/30 rounded-lg border border-dungeon-700/50">
                            {(() => {
                                const AlignmentIcon = getAlignmentIconById(item.id);
                                return <AlignmentIcon className="h-8 w-8 text-gold-500" />;
                            })()}
                            <div>
                                <p className="text-sm font-medium text-dungeon-300">Código</p>
