...
import { getAlignments, type Alignment } from '@/lib/services/alignmentService.client';
import { CharacterRace } from '@/lib/types/character';
import type { CharacterClass as ClassDetails } from '@/lib/services/classService.client';
...
import { AlignmentSelector } from '@/components/alignment/AlignmentSelector';
...
export default function BasicInfoSection({ showHelp = false, onContinue }: BasicInfoSectionProps = {}) {
  ...
  const [availableAlignments, setAvailableAlignments] = useState<Alignment[]>([]);
  ...
  const selectedClass = character.classes?.[0]?.class as ClassDetails | undefined;

  // Validaciones
  const isBasicInfoValid = !!character.name && character.name.trim().length > 0;
  const isRaceValid = !!character.race;
  const isClassValid = (character.classes?.length ?? 0) > 0;

  // Generador de nombres aleatorios
  const handleGenerateName = async () => {
    try {
      const raceSlug = selectedRace?.slug || 'human';
      const response = await fetch(`/api/generate-name?race=${raceSlug}&count=1`);
      ...
      if (data.names && data.names[0]) {
        setCharacterName(data.names[0]);
      }
    } catch (error) {
      console.error('Error generating name:', error);
      // Fallback: generar un nombre genérico
      const fallbackNames = ['Aventurero', 'Héroe', 'Explorador', 'Viajero'];
      const randomFallback = fallbackNames[Math.floor(Math.random() * fallbackNames.length)];
      setCharacterName(randomFallback);
    }
  };
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            {/* Alineamiento */}
            <div>
              <label className="flex items-center gap-2 text-sm font-semibold text-gold-500 mb-2">
                Alineamiento
                <HelpTooltip
                  alwaysVisible={showHelp}
