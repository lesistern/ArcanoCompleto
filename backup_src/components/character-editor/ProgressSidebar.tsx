...
export default function ProgressSidebar({ character, className = '' }: ProgressSidebarProps) {
  ...
  const abilityScores = character.abilityScores?.current ?? { str: 0, dex: 0, con: 0, int: 0, wis: 0, cha: 0 };
  ...
  const progress = {
    ...
    abilities: Object.values(abilityScores).every(score => score > 0),
      ...
      <div className="space-y-4">
        ...
        <ProgressItem
          ...
          value={
            classes.length > 0 && classes[0]?.class
              ? `${classes[0].class.name} ${classes[0].level}`
              : 'No seleccionada'
          }
        />
        ...
        <ProgressItem
          ...
          value={
            progress.abilities
              ? 'Asignadas'
              : `${Object.values(abilityScores).filter(s => s > 0).length}/6 completas`
          }
        />
