...
async function main() {
  ...
  for (const classData of classes) {
    try {
      // Separar datos y mapear a columnas reales
      const { weapon_proficiencies, armor_proficiencies, class_skills, name, hit_die, skill_points_per_level, bab_progression, fortitude_save, reflex_save, will_save, description, ...rest } = classData;

      // Mapear a nombres de columnas reales en Supabase
      // Convertir valores en inglés a español para los ENUMs
      ...
      const classToInsert = {
        ...rest,
        titulo: name,
        dg: parseInt(hit_die.replace('d', '')),  // d8 -> 8
        skill_points_first_level_base: (skill_points_per_level * 4),  // En D&D 3.5, primer nivel es 4x
        skill_points_per_level_base: skill_points_per_level,
        bab: progressionMap[bab_progression] || bab_progression,
        fort: progressionMap[fortitude_save] || fortitude_save,
        ref: progressionMap[reflex_save] || reflex_save,
        will: progressionMap[will_save] || will_save,
        subtitulo: description
      };

      const { data, error } = await supabase
        .from('classes')
        .upsert(classToInsert, { onConflict: 'slug' })
        .select();
