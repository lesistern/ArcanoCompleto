...
async function checkModeratorAccess() {
  ...
  const { data: profile } = await supabase
    .from('profiles')
    .select('forum_trust_level')
    .eq('id', user.id)
    .single();

  if (profile?.forum_trust_level !== 'TL4') {
    redirect('/foro');
  }
...
async function loadModerationData() {
  ...
  const { data: stats } = await supabase
    .rpc('get_forum_moderation_stats')
    .single();
        ...
        <Link
          href={
            report.thread_id && !report.post_id
              ? `/foro/categoria/${report.thread_id}`
              : report.thread_id
              ? `/foro/categoria/${report.thread_id}#post-${report.post_id}`
              : '#'
          }
