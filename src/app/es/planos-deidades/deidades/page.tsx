...
import { Sparkles, Sword, Shield, Skull, Crown, Info, Search, X } from 'lucide-react';
    ...
    useEffect(() => {
        async function fetchDeities() {
            const { data, error} = await supabase
                .from('deities')
                .select('slug, name_es, rank, titles_es, portfolio_es, alignment, domains, symbol_es')
                .order('name_es');
                        ...
                        >
                            {searchResults.map((deity) => {
                                ...
                                return (
                                    <Link
                                        key={deity.slug}
                                        href={`/es/planos-deidades/deidades/${deity.slug}`}
                                        onClick={() => {
                                            setSearchTerm('');
                                            setShowDropdown(false);
                                        }}
                        ...
                        return (
                            ...
                            >
                                <div className="h-full bg-gradient-to-br from-dungeon-800/90 to-dungeon-900/90 rounded-lg border border-dungeon-700 overflow-hidden deity-card relative">
                                    {/* Icon Header con color de alineamiento */}
                                    <div className="relative h-32 w-full overflow-hidden">
                                        {/* Efecto líquido de fondo con color de alineamiento */}
                                        <div
