            ...
            <Card className="border-gray-700 bg-gray-900/50">
                ...
                <CardContent>
                    <div className="grid md:grid-cols-3 gap-4 text-sm text-gray-200">
                        <div className="bg-gray-950/30 p-3 rounded border border-gray-800/50 relative overflow-hidden">
                            <div className="absolute top-0 right-0 p-1 opacity-10"><Activity className="h-12 w-12" /></div>
                            <h4 className="font-bold text-gray-100 mb-1">Extraordinarias (Ex)</h4>
                            <p className="text-xs text-gray-300">Habilidades físicas extremas. <strong>No son mágicas</strong>. Funcionan donde sea.</p>
                        </div>
                        <div className="bg-gray-950/30 p-3 rounded border border-gray-800/50 relative overflow-hidden">
                            ...
                            <p className="text-xs text-gray-300">Como lanzar un hechizo pero sin componentes. <strong>Provocan AdO</strong> y se pueden disipar.</p>
                        </div>
                        <div className="bg-gray-950/30 p-3 rounded border border-gray-800/50 relative overflow-hidden">
                            ...
                            <p className="text-xs text-gray-300">Mágicas pero innatas (ej. aliento de dragón). <strong>No provocan AdO</strong> y no se disipan.</p>
                        </div>
                    </div>
                </CardContent>
            </Card>
                    ...
                    <CardContent className="space-y-4 text-gray-200 text-sm">
                        <div className="grid grid-cols-1 gap-2">
                            <div className="flex justify-between items-center border-b border-gray-800 pb-1">
                                <span className="font-bold text-gray-100">Vista Ciega</span>
                                <span className="text-[10px] text-green-400 bg-green-950/30 px-2 rounded">Radar perfecto</span>
                            </div>
                            <p className="text-xs text-gray-300 mb-2">Ves sin ojos. La invisibilidad y la oscuridad no te afectan.</p>

                            <div className="flex justify-between items-center border-b border-gray-800 pb-1">
                                <span className="font-bold text-gray-100">Sentido Ciego</span>
                                <span className="text-[10px] text-yellow-400 bg-yellow-950/30 px-2 rounded">Radar borroso</span>
                            </div>
                            <p className="text-xs text-gray-300 mb-2">Sabes dónde están, pero no los ves bien (tienen ocultación).</p>

                            <div className="flex justify-between items-center border-b border-gray-800 pb-1">
                                <span className="font-bold text-gray-100">Olfato</span>
                                <span className="text-[10px] text-orange-400 bg-orange-950/30 px-2 rounded">Rastreo</span>
                            </div>
                            <p className="text-xs text-gray-300">Detectas enemigos cercanos por el olor y puedes seguirlos.</p>
                        </div>
                    </CardContent>
                </Card>
            </div>
                ...
                <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                        {conditions.map((c, i) => {
                            const Icon = c.icon;
                            return (
                                <div key={i} className="bg-gray-950/20 p-2 rounded border border-gray-800/30 hover:bg-gray-900/40 transition-colors flex flex-col gap-1">
                                    <div className="flex items-center gap-2 mb-1">
                                        <Icon className={`h-3 w-3 ${c.color}`} />
                                        <h5 className="font-bold text-gray-100 text-xs">{c.name}</h5>
                                    </div>
