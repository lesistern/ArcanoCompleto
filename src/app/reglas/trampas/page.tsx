        ...
        <div className="container mx-auto px-4 py-8 max-w-6xl">
            ...
            <div className="mb-8">
                ...
                <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                        <h1 className="text-3xl md:text-4xl font-bold text-dungeon-100">Trampas</h1>
                        <p className="text-dungeon-400">Detección, desactivación y tipos de trampas</p>
                    </div>
                </div>
            </div>
                ...
                <CardContent className="p-6">
                    <div className="flex gap-3">
                        ...
                        <div className="text-dungeon-300 space-y-2">
                            <p>
                                Las <strong className="text-dungeon-100">trampas</strong> son peligros ocultos diseñados
            ...
            <section className="mb-8">
                <h2 className="text-2xl font-bold text-dungeon-100 mb-4">Tipos de Trampas</h2>
                    ...
                    {TRAP_TYPES.map((type) => (
                        <Card key={type.name} className="bg-dungeon-900/50 border-dungeon-700">
                            ...
                            <CardContent className="space-y-3">
                                <p className="text-sm text-dungeon-300">{type.description}</p>
                                ...
                                <div className="pt-2 border-t border-dungeon-700 space-y-1 text-sm">
                    ...
                    <CardContent className="p-6 space-y-4">
                        ...
                        <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-3">
                            {DETECTION_DCS.map((item) => (
                                <div key={item.dc} className="bg-dungeon-800/50 p-3 rounded-lg border border-dungeon-700">
                                    <div className="text-lg font-bold text-gold-400 mb-1">CD {item.dc}</div>
                                    <div className="text-sm text-dungeon-200 font-medium">{item.difficulty}</div>
                                    <div className="text-xs text-dungeon-400 mt-1">{item.example}</div>
                                </div>
                            ))}
                        </div>
            ...
            {/* Trap Elements */}
            <section className="mb-8">
                <h2 className="text-2xl font-bold text-dungeon-100 mb-4">Elementos de una Trampa</h2>
                <div className="grid md:grid-cols-2 gap-3">
                    {TRAP_ELEMENTS.map((el) => (
                        <Card key={el.element} className="bg-dungeon-900/50 border-dungeon-700">
                            <CardContent className="p-4">
                                <h3 className="font-semibold text-orange-400 mb-1">{el.element}</h3>
                                <p className="text-sm text-dungeon-300">{el.description}</p>
                            </CardContent>
                        </Card>
                    ))}
                </div>
            ...
            {/* CR Calculation */}
            <section>
                <h2 className="text-2xl font-bold text-dungeon-100 mb-4">Nivel de Desafío (ND)</h2>
                <Card className="bg-dungeon-900/50 border-dungeon-700">
                    <CardContent className="p-6">
                        ...
                        <ul className="text-sm text-dungeon-200 space-y-2">
